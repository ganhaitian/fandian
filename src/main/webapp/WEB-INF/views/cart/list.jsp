...
<div class="container" id="mainContainer">
        ...
        <ul class="list-group">
                ...
                <li id="dish_${dish.dish.id}_${dish.taste.id}_${dish.weight.id}" class="list-group-item dish-entry" data-id="${dish.dish.id}" data-name="${dish.dish.name}" data-count="${dish.number}" data-fee="${dish.dish.price}" data-taste="${dish.taste.id}" data-weight="${dish.weight.id}">
                    ...
                    <p class="tools-bar hide">
                        ...
                        <span class="pull-right">
                            <button class="btn btn-default btn-oper-minus" data-id="${dish.dish.id}" data-price="${dish.dish.price}" data-taste="${dish.taste.id}" data-weight="${dish.weight.id}" data-ratio="${dish.weight.price_ratio}"><i class="fa fa-minus-circle"></i></button>
                            <button class="btn btn-default btn-oper-plus" data-id="${dish.dish.id}" data-price="${dish.dish.price}" data-taste="${dish.taste.id}" data-weight="${dish.weight.id}" data-ratio="${dish.weight.price_ratio}"><i class="fa fa-plus-circle"></i></button>
                            <button class="btn btn-default  btn-oper-remove" data-id="${dish.dish.id}" data-price="${dish.dish.price}" data-taste="${dish.taste.id}" data-weight="${dish.weight.id}" data-ratio="${dish.weight.price_ratio}"><i class="fa fa-trash-o"></i></button>
                        </span>
                    </p>
                </li>
            </c:forEach>
            ...
            <c:if test="${(!existedBill && isCustomer) || !isCustomer}">
                <li class="list-group-item">
                    <a href="<c:url value="/dish/category"/>"><h4><i class="fa fa-cutlery"></i>&nbsp;&nbsp;再点点儿</h4></a>
                </li>
            </c:if>
    ...
    <div id="tip-no-dish" class="alert alert-danger" style="display: none;">
        <i class="fa fa-info-circle"></i> 您还没有点菜 :(
    </div>
...
<script type="application/javascript">
    ...
    $(function(){
        ...
        //minus btn  click
        $('.btn-oper-minus').on('click',function(){
            var dish_id = $(this).data('id'),
                dish_price = $(this).data('price'),
                dish_price_ratio = parseFloat($(this).data('ratio')),
                dish_taste = $(this).data('taste'),
                dish_weight = $(this).data('weight'),
                dish_number = parseFloat($('#dish_number_'+dish_id+'_'+dish_taste+'_'+dish_weight).text()),
                sum_fee = parseFloat($('#bill_sum_fee').text());
            if (dish_number > 1){
                $.ajax({
                    ...
                    success:function(data){
                        if(data.success){
                            ...
                            $('#bill_sum_fee').text(sum_fee-dish_price*dish_price_ratio);
                        }
                    }
                });
            }else{
                $.ajax({
                    url:"<c:url value="/cart/dishremove"></c:url>",
                    type:"POST",
                    dataType:"JSON",
                    data: {"dishId":dish_id,"taste":dish_taste,"weight":dish_weight},
                    success:function(data){
        ...
        //plus dish btn
        $('.btn-oper-plus').on('click',function(){
            var dish_id = $(this).data('id'),
                dish_price = parseFloat($(this).data('price')),
                dish_price_ratio = parseFloat($(this).data('ratio')),
                dish_taste = $(this).data('taste'),
                dish_weight = parseFloat($(this).data('weight')),
                dish_number = parseFloat($('#dish_number_'+dish_id+'_'+dish_taste+'_'+dish_weight).text()),
                sum_fee = parseFloat($('#bill_sum_fee').text());
        ...
        //remove dish btn
        $('.btn-oper-remove').on('click',function(){
            var dish_id = $(this).data('id'),
                dish_price = $(this).data('price'),
                dish_price_ratio = parseFloat($(this).data('ratio')),
                dish_taste = $(this).data('taste'),
            ...
            $.ajax({
                ...
                success:function(data){
                    if(data.success){
                        ...
                        $('#bill_sum_fee').text(sum_fee-dish_price*dish_price_ratio*dish_number);
                    }
                }
            });
        ...
        $("#confirm-bill").click(function(){
            ...
            if(billDetails.length == 0){
                $('#tip-no-dish').fadeIn();
                setTimeout(function(){$('#tip-no-dish').fadeOut();},3000);
                return;
            }
